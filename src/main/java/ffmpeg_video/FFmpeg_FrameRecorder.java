...
public class FFmpeg_FrameRecorder implements AutoCloseable, PlugInFilter {
	...
	static final String[] encoders = new String[] { "by format", "mpeg4", "h264", "h265", "mjpeg", "huffyuv",
			"custom" };
	static final int defaultEncoderIndex = 0; // automatic selection by format
	static final int[] encCodes = new int[] { AV_CODEC_ID_NONE, AV_CODEC_ID_MPEG4, AV_CODEC_ID_H264, AV_CODEC_ID_H265,
			AV_CODEC_ID_MJPEG, AV_CODEC_ID_HUFFYUV, -1 };
...
//		}
	...
	static void fillFormatsAndEncoders() {
		...
		if (ffmpegEncoders == null || ffmpegEncoders.size() == 0 || ffmpegFormats == null
				|| ffmpegFormats.size() == 0) {
	...
	private boolean showDialog() {
		...
		if (!IJ.isMacro()) {
			...
			if (initialFormatIndex >= formats.length)
				initialFormatIndex = defaultFormatIndex;
			initialEncoderIndex = (int) Prefs.get("ffmpegvideoimport.savedEncoderIndex", defaultEncoderIndex);
			if (initialEncoderIndex >= encoders.length)
				initialEncoderIndex = defaultEncoderIndex;
		...
		btn_vopt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (vcodecOptTab == null) {
					...
					JScrollPane scrollPane = new JScrollPane(vcodecOptTab, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		btn_getCompatibleFormats.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (encChoiceInd > 0) {
					...
					if (enc != null) {
						IJ.log("=========================================================================");
						IJ.log("List of compatible formats for the encoder " + enc.name().getString() + " ("
								+ enc.long_name().getString() + ")");
						IJ.log("-------------------------------------------------------------------------");
						int id = enc.id();
						for (AVOutputFormat oformat : ffmpegFormats) {
							...
							if (r[2] == 1) {
								...
								String longname = oformat.long_name() != null
										? " - " + oformat.long_name().getString() + "; "
										: "; ";
								String extensions = oformat.extensions() != null
										? oformat.extensions().getString() + "; "
										: " NO DEFAULT EXTENSION; ";
								AVCodec defEnc = avcodec_find_encoder(oformat.video_codec());
								String defaultCodec = "";
								if (defEnc != null) {
									String defEncName = defEnc.name() != null ? " " + defEnc.name().getString() : " ";
									String defEncLongname = defEnc.long_name() != null
											? " - " + defEnc.long_name().getString() + ";"
											: ";";
		...
		if (firstSlice < 1)
			firstSlice = 1;
		if (lastSlice > stack.getSize())
			lastSlice = stack.getSize();
		...
		if (!IJ.isMacro()) {
			...
			if (Recorder.record) {
				if (!optLine.isEmpty())
					Recorder.recordOption("additional_encoder_options", optLine);
			}
		} else {
		...
		String codec = "";
		if (encCodes[codecCode] != 0)
			codec = "[" + (encCodes[codecCode] < 0 ? customVEnc
					: avcodec_find_encoder(encCodes[codecCode]).name().getString()).toUpperCase() + "]";
		...
		if (suggestedExtension.isEmpty())
			suggestedExtension = defaultFormatForGuess;
//		if (formatCode>0) {
...
//		}
			...
			if (IJ.isMacro()) {
				...
				if (macroOptions != null) {
					...
					if (!path.isEmpty()) {
						...
						String newpath = path.substring(0, path.lastIndexOf(extension) - 1) + "-" + timestamp + "."
								+ extension;
						Macro.setOptions(macroOptions.substring(0, macroOptions.indexOf(path)) + newpath
								+ macroOptions.substring(macroOptions.indexOf(path) + path.length()));
					}
				}
			}
		}
		...
		if (!IsEncoderCompatible(suggestedExtension, codecCode, customVEnc)) {
			IJ.showMessage("Error", "Selected encoder " + codec + " cannot be used with the selected output format ["
					+ suggestedExtension.toUpperCase() + "]");
			return false;
		}
	...
	/**
	 * Suggests file extension for a combination of format (formatCode) and encoder
	 * (codecCode or customVEnc). Returns empty string if the extension should be
	 * defined by later, at save dialog.
	 * 
	 * @param formatCode
	 * @param codecCode
	 * @param customVEnc
	...
	String getSuggestedExtension(int formatCode, int codecCode, String customVEnc) {
		...
		if (formatCode > 0) {
			if (!formats[formatCode].equals(formatByExt))
				suggestedExtension = formats[formatCode].toLowerCase();
		} else {
			suggestedExtension = defaultFormats.get(encoders[codecCode]);
			if (encCodes[codecCode] == -1) {
				LinkedHashSet<String> compatExt = new LinkedHashSet<String>();
				...
				for (AVOutputFormat oformat : ffmpegFormats) {
					...
					if (r[2] == 1) {
						String longname = oformat.long_name() != null ? oformat.long_name().getString() : "";
						if (longname.toLowerCase().indexOf("raw ") != -1)
							continue;
						String extensions = oformat.extensions() != null ? oformat.extensions().getString() : " ";
						compatExt.addAll(Arrays.asList(extensions.split(",")));
					}
	...
	/**
	 * Encodes a stack into a video with default and specified parameters frame
	 * dimensions are resized proportionally to give the desired width The function
	 * uses standard working scheme: 1. InitRecorder 2. EncodeFrame in a cycle
	 * running through the specified stack range 3. StopRecorder
	 * 
	 * @param path the path of the resulting video file
	 * 
	 */
	public void recordVideo(String path, ImagePlus imp, int desiredWidth, double frameRate, int bRate, int firstSlice,
			int lastSlice) {
		if (imp == null)
			return;
		...
		if (!initRecorder(path, imp.getWidth(), imp.getHeight(), desiredWidth, frameRate, bRate, vFormat, codecCode,
				customVEnc, vKeys, vOptions))
			return;
	...
	static boolean isEncoderCompatible(String format, int enc_id) {
		...
		if (oformat == null)
			return false;
		int compatibility = getEncoderFromatCompliance(enc_id, oformat)[2];// avformat_query_codec(oformat, enc_id,
																			// FF_COMPLIANCE_NORMAL);
		if (compatibility < 0)
			IJ.log("Warning: Format/encoder compatibility is unknown.");
	...
	/**
	 * Initializes and starts FFmpegFrameRecorder with default settings: framerate =
	 * 25 fps, bitrate (automatically estimated to give high quality), video codec
	 * (MPEG-4 simple profile), video format "avi", pixel format YUV420P, gop size =
	 * 10, and other codec options are defaults. Video frame is proportionally
	 * rescaled from the initial dimensions of srcImp to give desired frame width.
	...
	/**
	 * Initializes and starts FFmpegFrameRecorder with default settings: video codec
	 * (MPEG-4 simple profile), video format "avi", pixel format YUV420P, gop size =
	 * 10, and other codec options are defaults. Video frame is proportionally
	 * rescaled from the initial dimensions of srcImp to give desired frame width.
	 * Video frame dimensions are aligned to 8 pixel.
	 ...
	 * 25 fps, bitrate (automatically estimated to give high quality), video codec
	 * (MPEG-4 simple profile), video format "avi", pixel format YUV420P, gop size =
	 * 10, and other codec options are defaults. Video frame is rescaled from
	 * dimensions of initial image to give desired frame width and height. Video
	 * frame dimensions are aligned to 8 pixel.
	 * 
	 * @param path    path to the resulting video file
	 * @param vWidth  desired width of video frame.
	 ...
	 * (MPEG-4 simple profile), video format "avi", pixel format YUV420P, gop size =
	 * 10, and other codec options are defaults. Video frame is proportionally
	 * rescaled from the specified initial dimensions to give desired frame width.
	 * Video frame dimensions are aligned to 8 pixel.
	 * 
	 * @param path      path to the resulting video file
	...
	public boolean initRecorder(String path, int vWidth, int vHeight, double frameRate, int bRate) {
		...
		return initialized = initRecorder(path, videoWidth, videoHeight, AV_PIX_FMT_NONE, frameRate, bRate,
				AV_CODEC_ID_MPEG4, null, "avi", 10, null, null);
	}

	/**
	 * Initializes and starts FFmpegFrameRecorder with default pixel format YUV420P,
	 * while other settings are customized: video format (0 is by file extension),
	 * video codec (0 is by format), custom codec can be specified, as well as
	 * custom codec options. Video frame is proportionally rescaled from the
	 * specified initial dimensions to give desired frame width. Video frame
	 * dimensions are aligned to 8 pixel.
	 * 
	 * @param path        path to the resulting video file
	 * @param srcWidth    width of initial image
	 * @param srcHeight   height of initial image
	 ...
	 */
	private boolean initRecorder(String path, int srcWidth, int srcHeght, int vWidth, double frameRate, int bRate,
			String video_format, int codec_code, String v_codec_custom, ArrayList<String> vKeys,
			ArrayList<String> vOptions) {
		if (vWidth < 8) {
			IJ.log("Incorrect output width");
			initialized = false;
			return false;
...
//		videoHeight = frameHeight;
		...
		return initialized = initRecorder(path, videoWidth, videoHeight, AV_PIX_FMT_NONE, frameRate, bRate, v_codec,
				v_codec_custom, video_format, 0, vKeys, vOptions);
	}

	/**
	 * Initializes and starts FFmpegFrameRecorder with customized settings: Video
	 * frame dimensions are aligned to 8 pixel.
	 * 
	 * @param path      path to the resulting video file
	 * @param vWidth    desired width of video frame.
	 ...
	 * @param vOptions  a list of corresponding options
	 */
	public boolean initRecorder(String path, int vWidth, int vHeight, int pixFmt, double frameRate, int bRate,
			int vcodec, String vcodecName, String vFmt, int gopSize, ArrayList<String> vKeys,
			ArrayList<String> vOptions) {
		...
		if (pixFmt >= 0)
			recorder.setPixelFormat(pixFmt);
		else {
	...
	/**
	 * Stops record and releases resources. Should be called at the end of record.
	 */
	...
	/**
	 * Encodes one frame (next frame of the video) The image will be transformed to
	 * RGB if necessary, then encoded with parameters specified in a
	 * InitRecorder(...) function
	 * 
	 * @param ip ImageProcessor of the image to encode
	 */
	public void encodeFrame(ImageProcessor ip) {
		if (frameHeightBorder != 0 || frameWidthBorder != 0)
			frame_ARGB = converter.convert(
...
//											0, frameHeightBorder/2)).convertToRGB().getBufferedImage());
			...
			frame_ARGB = converter.convert(ip.convertToRGB().getBufferedImage());

		try {
			recorder.recordImage(frame_ARGB.imageWidth, frame_ARGB.imageHeight, Frame.DEPTH_UBYTE, 4,
					frame_ARGB.imageStride, AV_PIX_FMT_ARGB, frame_ARGB.image);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	}
...
class SetupDialog extends NonBlockingGenericDialog {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (Macro.getOptions() == null && componentsAreSet() && source == getButtons()[0]) {
			...
			if (FFmpeg_FrameRecorder.encCodes[codecCode] != 0)
				codec = "[" + (FFmpeg_FrameRecorder.encCodes[codecCode] < 0 ? customVEnc
						: avcodec_find_encoder(FFmpeg_FrameRecorder.encCodes[codecCode]).name().getString())
						.toUpperCase() + "]";
			...
			if (!FFmpeg_FrameRecorder.formats[formatCode].equals(FFmpeg_FrameRecorder.formatByExt)) {
				...
				if (!FFmpeg_FrameRecorder.IsEncoderCompatible(suggestedExtension, codecCode, customVEnc)) {
					IJ.showMessage("Error",
							"Selected encoder " + codec + " cannot be used with the selected output format ["
									+ suggestedExtension.toUpperCase() + "]");
					return;
				}
			}
	...
	boolean componentsAreSet() {
		return codecChoice != null && formatChoice != null && customEncField != null && firstSliceField != null
				&& lastSliceField != null;
	}
